...
export default class AddPost extends Component {
    ...
    render() {
        const image = this.state?.uri ? {uri: this.state?.uri} : require('./images/imagePlaceholder.png');
        return (
            <ScrollView
                style={{flex: 1}}>
                <View style={{flex: 1, flexDirection: "row", margin: 20}}>
                    <Image
                        ...
                        source={image}/>
                    <Input
                        placeholder="Enter organization name" onChangeText={(text) => {this.setState({orgName: text})}} />
                </View>
